...
1. **Start the proxy** (`python3 start_proxy.py`)
2. **Configure OpenClaw** (`python3 configure_openclaw.py --enable`)
3. **Use OpenClaw normally** - keys rotate automatically behind the scenes
...
- ✅ **Cyclical rotation** - When key 6 rate-limited, cycles back to key 1
...
```bash
# 1. Start the proxy
python3 ~/.openclaw/skills/keymaster/scripts/start_proxy.py

# 2. Configure OpenClaw to use proxy
python3 ~/.openclaw/skills/keymaster/scripts/configure_openclaw.py --enable

# 3. Use OpenClaw normally
openclaw "create me a complex dashboard"
# Works for 60+ minutes, rotating through all 6 keys!
```

See **Proxy Setup** section below for details.
...
### Step 5: Set Up the Proxy (Critical for Long Tasks!)

**For OpenClaw itself to rotate keys**, you need to start the proxy server:

```bash
# Start the proxy (runs in foreground, press Ctrl+C to stop)
python3 ~/.openclaw/skills/keymaster/scripts/start_proxy.py

# Or run as daemon (background)
python3 ~/.openclaw/skills/keymaster/scripts/start_proxy.py --daemon

# Check if proxy is running
python3 ~/.openclaw/skills/keymaster/scripts/start_proxy.py --status

# Stop the proxy
python3 ~/.openclaw/skills/keymaster/scripts/start_proxy.py --stop
```

### Step 6: Configure OpenClaw to Use Proxy

```bash
# Route OpenClaw through the proxy
python3 ~/.openclaw/skills/keymaster/scripts/configure_openclaw.py --enable
...
This changes OpenClaw's config from:
- `https://integrate.api.nvidia.com/v1` → `http://127.0.0.1:8787`

**Now OpenClaw will automatically rotate keys!**
...
### Step 6: Start the Proxy (For Long Tasks!)

**This is the critical step that makes OpenClaw rotate keys automatically.**
...
### Step 7: Configure OpenClaw to Use Proxy
...
This changes OpenClaw's base URL from `https://integrate.api.nvidia.com/v1` to `http://127.0.0.1:8787`.

### Step 8: Use OpenClaw Normally

```bash
# Now OpenClaw will automatically rotate through all 6 keys!
openclaw "create me a complex dashboard with 10 widgets"

# Works for 60+ minutes, cycling through keys automatically
```

### How It Works

```
Before (stops at 10 mins):
  You → OpenClaw → NVIDIA (Key 1) → 429 Rate Limit → ❌ STOPS
...
After (works 60+ mins):
  You → OpenClaw → Proxy (localhost:8787)
              ...
              Key 1 works 10 mins → 429
                     ↓
              Key 2 works 10 mins → 429
                     ↓
              Key 3 works 10 mins → 429
                     ↓
              ...cycles through all 6 keys...
                     ↓
              If all cooling, waits and cycles back to Key 1
...
# View logs
tail -f ~/.openclaw/keymaster_proxy.log

# Stop proxy
python3 ~/.openclaw/skills/keymaster/scripts/start_proxy.py --stop

# Disable (revert to direct NVIDIA)
